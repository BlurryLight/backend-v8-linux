...
if [ "$VERSION" == "11.8.172" ]; then 
    ...
    sudo apt update
    sudo apt install -y clang-17 libc++-17-dev libc++abi-17-dev lld-17
    ln -sf /usr/lib/llvm-17 ~/customclang

    gn gen out.gn/x64.release --args="is_debug=false v8_enable_i18n_support=false v8_use_snapshot=true v8_use_external_startup_data=false v8_static_library=true strip_debug_info=true symbol_level=0 libcxx_abi_unstable=false v8_enable_pointer_compression=false v8_enable_sandbox=false $CXX_SETTING is_clang=true v8_enable_maglev=false v8_enable_webassembly=false clang_use_chrome_plugins=false use_sysroot=false use_glib=false clang_base_path=\"${HOME}/customclang\""

    node $GITHUB_WORKSPACE/node-script/linux_v8_v11_clang17_patch.js .
